...
function completeDomain(domain) {
  //if(domain.startsWith('0x'))return domain
...
function useCheckDomain(domain, invertCheck = false) {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const available = domain.startsWith('0x') ? false : await isEnsDomainAvailable(  //## eth addreses are never available(=can be opened)
          networkType,
          web3,
